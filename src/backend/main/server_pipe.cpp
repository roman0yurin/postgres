...
public:
    ...
    ~ParallelRunner()
    {
        remove_old_shared_memory(shm_key);
        if(client_terminated)
            exchanger.~Exchanger();
    }
...
key_t init_processor(Processor processor, const char* lock_path, std::string& live_lock_name, std::string& error)
{
    return wrapper(nullptr).init(processor? processor : invoke, lock_path, live_lock_name, error);
}
...
Datum c60_server_pipe_params(PG_FUNCTION_ARGS)
{
    ...
    key_t key = wrapper(nullptr).init(invoke, client_lock_path, live_lock_name, error);
    if(key == -1)
